...
var firebaseConfig = {
    ...
    var tripRequestRef = firebase.database().ref('requests');

    tripRequestRef.on('value', async function(snapshot) {
        var data = snapshot.val();

        await loadTripIcons(data);
    });
    ...
    var map = new google.maps.Map(document.getElementById('map'), {
        ...
        for (var key in fliter_icons) {
            ...
            div.innerHTML = '<img src="' + icon + '" style="margin-top:2px;margin-bottom:2px;"> ' + name;
    ...
    function loadTripIcons(data){
        var geocoder = new google.maps.Geocoder();

        Object.entries(data).forEach(([key, val]) => {
            console.log('val.pick_address',val.pick_address)
            geocoder.geocode({ 'address': val.pick_address }, function (results, status) {
                console.log('results',results);
                 var carIcon = new google.maps.Marker({
                    position: results[0].geometry.location,
                    //position: new google.maps.LatLng(30.7116,76.6866),
                    map: map,
                    icon: {
