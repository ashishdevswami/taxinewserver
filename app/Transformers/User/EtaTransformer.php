...
class EtaTransformer extends Transformer
{
    ...
    public function transform(ZoneType $zone_type)
    {
        ...
        // }
        if (request()->is_later==true || request()->is_later==1) {
            $ride_type = 2;
        } else {
            $ride_type = 1;
        }
       ...
       // echo $ride_type; die();
        $type_prices = ZoneTypePrice::where('price_type', $ride_type)->first();

         // echo "<pre>"; print_r($type_prices); die();
        $distance_in_unit = 0;
        ...
        if (request()->has('drop_lat') && request()->has('drop_lng') && request()->drop_lat) {
           ...
           // echo "<pre>"; print_r($previous_pickup_dropoff); die();
        ...
        $response['total'] = $ride->total_price+$ride->total_price*5/100;
        $response['approximate_value'] = 1;
        $response['min_amount'] = $ride->total_price+$ride->total_price*5/100;
        $response['max_amount'] = $ride->total_price+$ride->total_price*5/100;
    ...
    {
         $originaldistance = $distance_in_unit; //echo "sdsd";
       // $distance_in_unit = $distance_in_unit-3;
        ...
        if($calculatable_distance < 0 ){
         ...
         $price_per_distance = $type_prices->price_per_distance; // die();
       ...
       $time_price = 0; //($dropoff_time_in_seconds / 60) * $type_prices->price_per_time;
