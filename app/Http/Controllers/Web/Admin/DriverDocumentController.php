...
class DriverDocumentController extends BaseController
{
    ...
    public function approveDriverDocument(Request $request)
    {
        ...
        $this->toggleApprove($driver, $status,$request->driver_type);

        return 'success';
    }


    public function toggleApprove(Driver $driver, $status,$drivertype)
    {
        $status = $status == true ? 1 : 0;

        //$this->database->getReference('drivers/'.$driver->id)->update(['approve'=>(int)$status,'updated_at'=> Database::SERVER_TIMESTAMP]);

        if($drivertype!=""){ $drivertype = $drivertype;} else { $drivertype =1;}
        $driver->update([
            'approve' => $status,
