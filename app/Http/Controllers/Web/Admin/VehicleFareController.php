...
class VehicleFareController extends Controller
{
    ...
    public function create() 
    {
        ...
        $vechiclestypes = VehicleType::where('active', 1)->active()->get();
        //echo "<pre>"; print_r($vechiclestypes); die();
        $main_menu = 'vehicle-fare';
        $sub_menu = '';

        return view('admin.vehicle_fare.create', compact('page', 'main_menu', 'sub_menu', 'zones','vechiclestypes'));
    }
   ...
   public function store(AssignZoneTypeRequest $request)
    {
        ...
        $zoneType->zoneTypePrice()->create([
            ...
            'driver_base_price' => $request->driver_base_price ? $request->driver_base_price : 0.00,
            'driver_price_per_distance' => $request->driver_price_per_distance ? $request->driver_price_per_distance : 0.00,
            'driver_base_distance' => $request->driver_base_distance ? $request->driver_base_distance : 0.00,
            'driver_minimum_waiting_time' => $request->driver_minimum_waiting_time ? $request->driver_minimum_waiting_time : 0.00,
        ]);
        ...
        $zoneType->zoneTypePrice()->create([
            ...
            'driver_base_price' => $request->driver_base_price1 ? $request->driver_base_price1 : 0.00,
            'driver_price_per_distance' => $request->driver_price_per_distance1 ? $request->driver_price_per_distance1 : 0.00,
            'driver_base_distance' => $request->driver_base_distance1 ? $request->driver_base_distance1 : 0.00,
            'driver_minimum_waiting_time' => $request->driver_minimum_waiting_time1 ? $request->driver_minimum_waiting_time1 : 0.00,
        ]);
