...
class DriverArrivedController extends BaseController
{
    ...
    */
    public function arrivedRequest(Request $request)
    {
        ...
        if($request_detail->driver_id!=""){
            $driver = Driver::find($request_detail->driver_id);
            $title = trans('push_notifications.driver_arrived_title',[],$user->lang);
            $body = 'Your Booked JAG No '.$driver->car_number.' has arrived, You will be timed out after 5 mins';
        }else {
            $title = trans('push_notifications.driver_arrived_title',[],$user->lang);
            $body = 'Your Booked JAG has arrived, You will be timed out after 5 mins';
        }
