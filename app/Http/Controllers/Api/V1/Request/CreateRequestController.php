...
class CreateRequestController extends BaseController
{
    ...
    */
    public function createRequest(CreateTripRequest $request)
    {
       ...
       // $getNewdata = $this->distance($request->input('pick_lat'), $request->input('pick_lng'),$request->input('drop_lat'), $request->input('drop_lng'),'K');
        //echo "<pre>"; print_r($getNewdata); die();
        $request_params['total_distance'] = $request->input('distance');
        $request_params['total_time'] = $request->input('time');
        ...
        // $datarr = array();
        // $datarr['to'] = $notifable_driver->fcm_token;
        // $notificaitonar = array();
        // $notificaitonar['title'] = $title;
        // $notificaitonar['body'] = $body;
        // $datarr['notification'] = $notificaitonar;
        // $data = json_encode($datarr);
        // //FCM API end-point
        // $url = 'https://fcm.googleapis.com/fcm/send';
        // //api_key in Firebase Console -> Project Settings -> CLOUD MESSAGING -> Server key
        // $server_key = 'AAAAexGWyGM:APA91bExEv6jSPc1f51br7jD-eOJzHTdGLOUGMiS1gGD1STGrqCOxcmwIrRIg4R-7VHXVq4CHFD9HeSzr2MrOUsyltBtglZbXyNX7D9S_6n1aarVX3w6-JF_hGgQfx5ByQHPyBT_91lk';
        ...
        //$notifable_driver->notify(new FCMPushNotification($title, $body,$notifable_driver->fcm_token));
       // die();
    ...
    public function createadminRequest(CreateTripRequest $request)
    {
        /**
        * Check if the user has registred a trip already
        * Validate payment option is available.
        ...
        */
        ...
        // @TODO
        //Check if the user created a trip and waiting for a driver to accept. if it is we need to cancel the exists trip and create new one
        $request_meta_with_current_user = RequestMeta::where('user_id', $request->user_id);
       ...
       // echo "<pre>"; print_r($user_detail); die();
        ...
        $request_number = 'REQ_'.sprintf("%06d", $request_number+1);
        $tripstartTime = '';
        if($request->is_later==1){
            $dtimeate = $request->time;
            $newTime = date('H:i:s', strtotime($dtimeate. ' -330 minutes'));
            $tripstartTime = $request->date.' '.$newTime;
        }else {
            $request->is_later = 0;
        }
        ...
        $request_params['company_key'] = $user_detail->company_key;
        ...
        $request_result =  fractal($request_detail, new TripRequestTransformer)->parseIncludes('userDetail');
        //echo $request->vehicle_type; die();
        //Log::info($nearest_drivers);
        $nearest_drivers = Driver::where('id', $request->driver_id)->get();
        // Send Request to the nearest Drivers
         if (!$nearest_drivers) {
                goto no_drivers_available;
            }
        ...
        $pus_request_detail = $request_result->toJson();
        //echo "<pre>"; print_r($pus_request_detail); die();
        $push_data = ['notification_enum'=>PushEnums::REQUEST_CREATED,'result'=>$pus_request_detail];
       ...
       // die();
        ...
        return redirect('https://tagxi-server.atyarealtors.com/scheduled-rides')->with('success', 'created_request_successfully');
        //return $this->respondSuccess($request_result, 'created_request_successfully');
    }
    
    public function createadminpaid($id)
    {
        //$id = $request->id;
        Request::where('id', $id)->update(['is_paid'=>1]);
        echo "success"; die();
    }
    
    public function updatedriverdata($id,$driverid)
    {
        //$id = $request->id;
        Request::where('id', $id)->update(['driver_id'=>$driverid]);
        echo "success"; die();
    }
    ...
    public function getlatestdata()
    {
        ...
        $query = Request::whereNull('driver_id')->where('created_at','>',$newdate)->get();
    ...
    */
    public function getFirebaseDrivers($request, $type_id)
    {
        ...
        $fire_drivers = $this->database->getReference('drivers')->getValue();
        ...
        foreach ($fire_drivers as $key => $fire_driver) {
           ...
           // if(array_key_exists('vehicle_type',$fire_driver) && $fire_driver['vehicle_type']==$vehicle_type && $fire_driver['is_active']==1 && $fire_driver['is_available']==1 && $conditional_timestamp < $driver_updated_at){
                ...
                //}
                //echo "<pre>"; print_r($fire_driver['id']); die();
                // if(@isset($firebase_drivers[$fire_driver['id']])){
                  $firebase_drivers[$fire_driver['id']]['distance']= 25;//$distance;
                // }
            //}
   ...
   //die();
        ...
        //echo "<pre>"; print_r($firebase_drivers); die();
